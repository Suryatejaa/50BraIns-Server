...
// Fine-grained limiter for suggest/lookup endpoints (onChange)
const suggestLimiter = rateLimit({
    windowMs: 10 * 1000, // 10 seconds
    max: 20,             // up to 2 req/s per IP
    message: { error: 'Too many suggest requests, please slow down' },
    standardHeaders: true,
    legacyHeaders: false,
    keyGenerator: (req) => `${req.ip}:${req.params?.clanId || ''}`
});
